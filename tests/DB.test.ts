...
describe(DB.name, () => {
  ...
  test('lexicographic buffer iteration order', async () => {
    ...
    // The `keysIterated` are "escaped"
    // The `keys` generated by `nodeCrypto.randomBytes` are not escaped
    // Therefore in order to compare we shall escape the `keys` into `keys_`
    const keys_ = keys.map((k) => utils.keyPathToKey([k]));
    expect(keys_).not.toStrictEqual(keysIterated);
    keys_.sort(Buffer.compare);
    expect(keys_).toStrictEqual(keysIterated);
    // Sort the original keys
  ...
  test('sublevel lexicographic iteration', async () => {
    ...
    for await (const [kP] of db.iterator({ values: false }, ['level1'])) {
      const k = utils.keyPathToKey(kP);
      // Keys are buffers due to key encoding
  ...
  test('iterating sublevels', async () => {
    ...
    results = [];
    for await (const [kP, v] of db.iterator(undefined, ['level1'])) {
      const k = utils.keyPathToKey(kP);
      results.push([k.toString(), JSON.parse(v.toString())]);
    }
    expect(results).toStrictEqual([
